...
from dataloader import custom_collate,BucketBatchSampler
...
def re_trainer(epochs,model,optimizer
            ...
            f1_score_record=[],epoch_record=0,seed_value=0,batches_trained_epoch=0,optimized_data=None):
  ...
  for epoch in range(epoch_record,epochs):
    ...
    set_seed(seed_value)
    sampler=RandomSampler(list(range(len(train_dataset))))
    batches=list(BucketBatchSampler(sampler, batch_size=config['train_batch_size'],
             drop_last=False,bucket_size_multiplier=5))
    ...
    if (restart is True) and (config['checkpoint_exists'] is True):
      ...
      train_dataloader=DataLoader(dataset=train_dataset,shuffle=False,num_workers=7,pin_memory=True,
                            collate_fn=custom_collate,batch_sampler=batches[batches_trained_epoch:])
      ...
      train_dataloader=DataLoader(dataset=train_dataset,collate_fn=custom_collate,
                batch_sampler=batches,num_workers=7,pin_memory=True)
    ...
    for context,question,start,end in tq:
      ...
      context,question,=context.to(device),question.to(device)
