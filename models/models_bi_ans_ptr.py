...
#-------------------------------------------------------------------------------------------------
attention_common=attention_1(config['hidden_units']).to(device) # added this part to handle common attention parameters.
...
attention_ans_ptr=attention_2(config['hidden_units']).to(device)

class decoder_2_forward(nn.Module):
  def __init__(self,hidden_units):
    super().__init__()
    self.hidden_units=hidden_units
    self.attention_2=attention_ans_ptr
...
class decoder_2_reverse(nn.Module):
  def __init__(self,hidden_units):
    super().__init__()
    self.hidden_units=hidden_units
    self.attention_2=attention_ans_ptr
...
class match_lstm(nn.Module):
  def __init__(self,encoder_c,encoder_q,decoder_forward,
                decoder_reverse,decoder_2_forward,decoder_2_reverse):
    super().__init__()
    self.encoder_c=encoder_c
    self.encoder_q=encoder_q
    self.decoder_forward=decoder_forward
    self.decoder_reverse=decoder_reverse
    self.decoder_ans_f=decoder_2_forward
    self.decoder_ans_r=decoder_2_reverse
  ...
  def forward(self,question,context):
    ...
    timesteps=context_hidden.size(1)    # timesteps in match-lstm , context -P=400
    ...
    hidden_match= torch.cat((hidden_set_f,hidden_set_r),dim=2)

    # BI-ANS-PTR NETWORK

    hidden_ans_f,mem_ans_f=torch.zeros(1,batch,hidden_size,device=device),torch.zeros(1,batch,hidden_size,device=device)
    attn_matrix_f=torch.empty(batch,timesteps,2,device=device)

    hidden_ans_r,mem_ans_r=torch.zeros(1,batch,hidden_size,device=device),torch.zeros(1,batch,hidden_size,device=device)
    attn_matrix_r=torch.empty(batch,timesteps,2,device=device)    

    for j in range(2):  # 2 is used considering the 2 outputs for start and end index
      
      hidden_ans_f,mem_ans_f,attn_weights_f=self.decoder_ans_f(hidden_match,hidden=hidden_ans_f,mem=mem_ans_f) # attn shape- batch ,1 ,timesteps
      hidden_ans_r,mem_ans_r,attn_weights_r=self.decoder_ans_r(hidden_match,hidden=hidden_ans_r,mem=mem_ans_r) # attn shape- batch ,1 ,timesteps

      attn_weights_f=attn_weights_f.transpose(1,2)
      attn_weights_f=attn_weights_f.squeeze(2)

      attn_weights_r=attn_weights_r.transpose(1,2)
      attn_weights_r=attn_weights_r.squeeze(2)      
      
      hidden_ans_f=hidden_ans_f.transpose(0,1)
      hidden_ans_r=hidden_ans_r.transpose(0,1)
      
      attn_matrix_f[:,:,j]=attn_weights_f # adding hidden state values for each timestep batch wise
      attn_matrix_r[:,:,1-j]=attn_weights_r

    resultant_attn_matrix=torch.div(torch.add(attn_matrix_f,attn_matrix_r),2)

    return resultant_attn_matrix
...
def load_model(hidden_units,device,weights,xavier_init=True):
  ...
  de_ans_f=decoder_2_forward(hidden_units).to(device) # answer pointer decoder
  de_ans_r=decoder_2_reverse(hidden_units).to(device)
  
  match_model=match_lstm(encoder_c=context_preprocess,encoder_q=question_preprocess,
                    decoder_forward=de_f,decoder_reverse=de_b,
                    decoder_2_forward=de_ans_f,decoder_2_reverse=de_ans_r).to(device)       
...
if __name__=='__main__':
  ...
  f=open('/home/pickle_objects/squad_weight.pkl','rb')
  trained_weights=pickle.load(f)
  trained_weight=trained_weights.float().to('cpu')  

  lo=load_model(150,'cuda',trained_weight)

  pytorch_all_params = sum(p.numel() for p in lo.parameters())
  pytorch_total_params = sum(p.numel() for p in lo.parameters() if p.requires_grad)
