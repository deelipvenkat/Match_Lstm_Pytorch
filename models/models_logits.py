"""module containing the match-lstm model returning logits as output"""
...
class attention_2(nn.Module):
  ...
  def forward(self,match_output,hidden):
    ...
    attn_weights,attn_log=self.score(match_output,hidden)
    
    attn_weights_1=attn_weights.transpose(1,2) # batch*1*question_length
    attn_logits_1=attn_log.transpose(1,2)

    attn_combination=torch.bmm(attn_weights_1,match_output) # shape= batch*1*hidden_units = 34,1,100
    
    return attn_weights_1,attn_combination,attn_logits_1  # returns a bmm product of attention weights & question hidden states.
  ...
  def score(self,match_output,hidden):
    ...
    return attn_weights,output_final # shape = batch*question_timestamps*1
...
class decoder_2(nn.Module):
  ...
  def forward(self,match_output,hidden,mem):
    ...
    _,attn_combination,attn_l=self.attention_2(match_output,hidden)  # context shape= (batch,1,hidden_units)
    
    output,(_,mem)=self.lstm_layer_2(attn_combination,(hidden,mem))

    return output,mem,attn_l
...
class match_lstm(nn.Module):
  ...
  def forward(self,question,context):
    ...
    attn_matrix=torch.empty(batch,timesteps,2,device=device)

    for j in range(2):  # 2 is used considering the 2 outputs for start and end index
      
      hidden_ans,mem_ans,attn_logits=self.decoder_ans(hidden_match,hidden=hidden_ans,mem=mem_ans) # attn shape- batch ,1 ,timesteps

      attn_logits=attn_logits.transpose(1,2)
      attn_logits=attn_logits.squeeze(2)
      
      hidden_ans=hidden_ans.transpose(0,1)
      attn_matrix[:,:,j]=attn_logits # adding hidden state values for each timestep batch wise
